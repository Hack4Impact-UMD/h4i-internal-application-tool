...
router.post("/application", [isAuthenticated, validateSchema(applicationResponseSchema)], async (req: Request, res: Response) => {
    ...
    try {
      ...
      const profileCollection = db.collection("applicantUserProfiles");
      ...
      await profileCollection.doc(newAppResponse.user_id).set(
        {
          applicationResponses: admin.firestore.FieldValue.arrayUnion(newAppResponse.id),
        },
        { merge: true }
      );
...
router.put("/application/:id", [isAuthenticated, validateSchema(applicationResponseSchema.partial())], async (req: Request, res: Response) => {
