...
router.post("/application", [isAuthenticated, validateSchema(newApplicationResponseSchema)], async (req: Request, res: Response) => {
    ...
    try {
      const newAppResponse: ApplicationResponse = {
        ...input,
        applicationResponseId: input.id,
        status: "in-progress",
        dateSubmitted: "",
        decisionLetterId: "",
      };
      ...
      logger.info(`Created ApplicationResponse with ID: ${newAppResponse.id} for user ${newAppResponse.userId}`);

      const userRef = profileCollection.doc(newAppResponse.userId);
      await userRef.set(
        {
...
router.put( "/application/:id", [isAuthenticated, validateSchema(newApplicationResponseSchema.partial())], async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (existingResponse.userId !== currentUserId) {
      ...
      if (["submitted", "reviewed"].includes(existingResponse.status)) {
        logger.warn(`Attempt to update ${existingResponse.status} response ${applicationId}`);
        return res.status(400).send(`Cannot update a ${existingResponse.status} application response`);
      }
