...
export default function SuperReviewerReviewersDashboard() {
  ...
  const { data: reviewers, isPending: reviewersPending, error: reviewersError } = useAllReviewers();
  const { data: reviewData, isPending: dataPending, error: dataError } = useReviewDataForForm(formId!)
  const { data: assignments, isPending: assignmentsPending, error: assignmentsError } = useReviewAssignmentsForForm(formId!)

  const { search } = useSearch();

  const [statusFilter, setStatusFilter] = useState<"all" | "complete" | "pending">("all")

  const numComplete = useMemo(() => reviewers?.reduce((acc, reviewer) => {
    const data = reviewData?.filter(d => d.reviewerId === reviewer.id) ?? [];
    const assigned = assignments?.filter(a => a.reviewerId === reviewer.id) ?? [];

    if (data.length === 0 && assigned.length === 0) return acc;

    if (data.reduce((acc, review) => review.submitted ? (acc + 1) : acc, 0) === assigned.length) {
      return acc + 1;
    } else {
      return acc;
    }
  }, 0), [assignments, reviewData, reviewers])

  if (!formId) return <p>No formId found! The url is probably malformed.</p>;
  if (reviewersError) return <p>Failed to fetch reviewers: {reviewersError.message}</p>;
  if (dataError) return <p>Failed to fetch review data: {dataError.message}</p>;
  if (assignmentsError) return <p>Failed to fetch review assignments: {assignmentsError.message}</p>;
  if (reviewersPending || dataPending || assignmentsPending || !reviewers) return <Loading />;
