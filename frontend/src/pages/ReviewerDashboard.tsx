...
export default function ReviewerDashboard() {
	...
	const { data: assignedApps, isLoading: assignmentsLoading, error: assignmentsError } = useMyReviewAssignments(formId ?? "")
	const { data: reviews, isLoading: reviewsLoading, error: reviewError } = useMyReviews(formId ?? "")
	...
	return <div className="w-full h-full bg-lightgray flex flex-col items-center p-2 py-4">
		<div className="max-w-5xl w-full rounded bg-white p-4 flex flex-col gap-2">
			<div className="flex flex-row items-center">
				...
				<input value={search} onChange={e => setSearch(e.target.value)} className="border border-gray-300 rounded-full px-2 py-1 text-sm min-w-sm ml-auto" placeholder="Search" />
			</div>
