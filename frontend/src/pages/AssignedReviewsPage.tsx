...
export function AssignedReviewsPage() {
	const [statusFilter, setStatusFilter] = useState<"reviewed" | "pending" | "all">("all")
	const { responseId } = useParams<{ responseId: string }>()
	const { data: assignedApps, isPending: assignmentsPending, error: assignmentsError } = useReviewAssignmentsForResponse(responseId!)
	const { data: applicant, isPending: applicantPending, error: applicantError } = useApplicantForResponse(responseId!)
	const { data: response, isPending: responsePending, error: responseError } = useApplicationResponse(responseId!)

	const numReviewed = 0;

	if (assignmentsPending || applicantPending || responsePending) return <Loading />

	if (assignmentsError) return <p>Failed to fetch assignments: {assignmentsError.message}</p>
	if (applicantError) return <p>Failed to fetch applicant: {applicantError.message}</p>
	if (responseError) return <p>Failed to fetch response: {responseError.message}</p>
	...
	return <div className="w-full h-full bg-lightgray flex flex-col items-center p-2 py-4">
		<div className="max-w-5xl w-full rounded bg-white p-4 flex flex-col gap-2">
			<div className="flex flex-row items-start gap-24 justify-start">
				<div>
					<p className="text-muted-foreground mb-2">NAME</p>
					<h1 className="text-2xl font-bold">{applicant.firstName} {applicant.lastName}</h1>
				</div>
				<div>
					<p className="text-muted-foreground mb-2">ROLES APPLIED</p>
					<div className="flex flex-wrap gap-1">
						{response.rolesApplied.map((role, i) => <ApplicantRolePill key={i} role={role} />)}
					</div>
				</div>
			</div>
