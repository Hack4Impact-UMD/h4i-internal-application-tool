...
const { id } = useParams();
console.log('ApplicantDetails - ID from URL:', id);
...
const handleResumeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const handleResumeUpload = async () => {
        ...
        if (!allowedTypes.includes(fileExtension)) {
            setUploadError('Invalid file format. Allowed formats are .pdf, .doc, .docx, .rtf');
            return;
        }
        ...
        // Create a reference to the resume file in Cloud Storage
        const storageRef = ref(storage, `resumes/${id}-${Date.now()}-${resumeFile.name}`);
        ...
        uploadTask.on(
            ...
            async () => {
                ...
                try {
                    // Update the applicant document in Firestore with the resume URL
                    await updateDoc(doc(db, 'users', id as string), {
                        resume: downloadURL,
                    });
                    setUploadSuccess(true);
                    setUploadError(null);
                    // Optionally, refresh the applicant data to show the resume link immediately
                    const updatedApplicant = { ...applicant, resume: downloadURL };
                    setApplicant(updatedApplicant as Applicant);
                } catch (error) {
                    console.error('Error updating applicant data with resume URL:', error);
                    setUploadError('Failed to save resume URL.');
                }
            }
        );
    };
...
<div className='question-answers'>
                        {applicant?.resume ? (
                            <a href={applicant.resume} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                                {applicant.resume}
                            </a>
                        ) : (
                            '-'
                    ...
                    <div className="mt-6">
                        <div className='question-headers'>Add/Update Resume</div>
                        <input
                            type="file"
                        ...
                        />
                        {resumeName && <p className="mt-2 text-sm text-gray-600">Selected file: {resumeName}</p>}
                        {uploadError && <p className="mt-2 text-red-500">{uploadError}</p>}
                        {uploadSuccess && <p className="mt-2 text-green-500">Resume uploaded successfully!</p>}
                        ...
                        <button
                            ...
                            className={`mt-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline ${
                                !resumeFile || uploadProgress > 0 ? 'opacity-50 cursor-not-allowed' : ''
                            }`}
                        >
                            {uploadProgress > 0 ? 'Uploading...' : 'Upload Resume'}
                        </button>
                        <p className="mt-2 text-xs text-gray-500">
                            Upload a file in .pdf, .doc, .docx, .rtf format with a maximum size of 2MB.
                        </p>
                    </div>
