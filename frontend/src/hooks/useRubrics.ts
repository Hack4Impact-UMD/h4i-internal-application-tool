...
import { getRoleRubricsForFormRole, uploadRubrics as uploadRubricsService } from "@/services/rubricService";
import { ApplicantRole, RoleReviewRubric } from "@/types/types";

export const useUploadRubrics = () => {
    return useMutation({
        mutationFn: ({ rubrics, token }: { rubrics: RoleReviewRubric[], token: string }) => uploadRubricsService(rubrics, token),
    });
};
