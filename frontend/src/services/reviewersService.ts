...
import { ApplicantRole, PermissionRole, ReviewerUserProfile } from "@/types/types"
...
export async function getReviewersForRole(role: ApplicantRole): Promise<ReviewerUserProfile[]> {
	const users = collection(db, USERS_COLLECTION);
	const q = query(users,
