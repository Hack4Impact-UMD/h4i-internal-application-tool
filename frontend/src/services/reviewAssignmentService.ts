import { ApplicantRole, AppReviewAssignment, ReviewerAssignment } from "@/types/types";
...
export async function assignReview(responseId: string, reviewerId: string, role: ApplicantRole) {
  const response = await getApplicationResponseById(responseId);

  if (!response) throw new Error(`Attempting to assign reviewer to non-existent response with id ${responseId}`);
...
export async function getReviewAssignments(formId: string, reviewerId: string): Promise<AppReviewAssignment[]> {
  const assignments = collection(db, REVIEW_ASSIGNMENT_COLLECTION)
  const q = query(assignments,
    and(
      where("formId", "==", formId),
...
export async function getReviewAssignmentById(assignmentId: string): Promise<ReviewerAssignment | undefined> {
  const assignments = collection(db, REVIEW_ASSIGNMENT_COLLECTION)
